...
-(int)match:(NSArray *)otherCards
{
    ...
    if (otherCards.count==2){
        ...
        if ([self similarity:matchedCard] == [self similarity:matchedCard]
            ...
            score += MATCHED_BONUS;
            self.operation = [NSString stringWithFormat:@"%@, %@, and %@ matched", self.contents, matchedCard.contents, matchingCard.contents];
        }else{
            self.operation = [NSString stringWithFormat:@"%@, %@, and %@ are not matched", self.contents, matchedCard.contents, matchingCard.contents];
        }
    }
    return score;
}
